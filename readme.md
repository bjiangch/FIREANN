...
- **PES model training** To investigate structure and electronic properties of electrochemical interface system using the FIREANN model and finite-field method, we first need to train an accurate and stable field-induced MLPES model. We provided a example task in */example/Au_NaCl/* to demonstrate how to train a FIREANN PES model using the **Au100_NaCl_Interface** atomic forces dataset.
- **Finite-field simulation** Then we need to use the trained PES model to perform finite-field MD simulations with LAMMPS or ASE. For LAMMPS compilation or the ASE interface with FIREANN model, please refer to the manual in main branch. We also provided a example task in */example_lammps/Au100_NaCl/* to demonstrate how to perform finite-field simulation using a FIREANN PES model.
- **MLEDR model training** Training the MLEDR model can directly utilize the atomic configurations dataset employed for the PES model training, but requires the additional saving of charge density file with and without applied electric field. We also provided a example task in */example/Au_NaCl_response/* to demonstrate how to train a FIREANN MLEDR model. Due to GitHub's file size upload limitations, we only provide the essential CP2k input parameter files, not the charge response file (CUBE in CP2k).
- **Structure and Electronic properties calculation** By employing the PES model for long-term, large-scale simulations and utilizing the MLEDR model to rapidly obtain charge response information of the system, we can simultaneously acquire structural details such as molecular orientation and ion distribution within the electrochemical interface system, alongside electrical properties including charge response and capacitance.
...
## More information about the Field-Induced Recursively embedded atom neural network for Charge Density and Response Branch
